...
export default function CreateDriver({ setTrigger, modalData, setModalOpen2 }) {
  ...
  const fullName = modalData?.user?.name;
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      vendorList?.forEach((el) => {
        if (el?.user?.username == modalData?.vender?.username) {
          setTempnumber(el?.id);
  ...
  async function RegisterDriver(values, resetForm) {
    ...
    try {
      const res = await axios.put(`/api/driver/author/${modalData?.id}/update/`,formData);
      setIsLoading(false);
